...
public class URIParserTest {
    ...
    public void getAllFull() {
        ...
        assertEquals(server,          URIParser.getServer(uri));
        assertEquals(database,        URIParser.getDatabase(uri));
        assertEquals(list(database),  URIParser.getDatabases(uri));
        assertEquals(tables,          URIParser.getTables(uri));
        assertEquals(list(c1,c2,c1),  URIParser.getColumns(tables,uri));
        assertEquals(list(join),      URIParser.getJoins(tables,uri));
        assertEquals(list(filter),    URIParser.getFilters(tables,uri));
        assertEquals(list(order),     URIParser.getOrders(tables,uri));
        assertEquals(list(group),     URIParser.getGroups(tables,uri));
        assertEquals(list(AggregateFunction.IDENTITY,AggregateFunction.IDENTITY,AggregateFunction.COUNT),
                     URIParser.getFunctions(tables,uri));
    }
    ...
    public void getDBsWhenOnlyServerWithSlash() {
        String uri = "/server/";
        assertEquals(list(),  URIParser.getDatabases(uri));
    }
